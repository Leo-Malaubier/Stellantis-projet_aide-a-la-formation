...
def generationXML(fichier):
    ...
    def determine(i,id):
        if ((id>=0 and id<=1) or (id>=9 and id <=10)):
            return "eli","red"
        elif id==5:
            return str(0),"green"
        ...
        if isinstance(tableau_etape[i][0],str):
            ...
            for j in range(len(tableau_tabPrincipale[i])):
                ...
                if isinstance(tableau_tabPrincipale[i][j],str):#on ne veux pas les nan qui sont des float (case vide)
                    print(str(parametres._parametreVideo))
                    for k in range(len(str(tableau_tabPrincipale[i][j]))):
                        ...
                        if variable=="":
                            ...
                            (tableau_tabPrincipale[i][j][k]=='n' and tableau_tabPrincipale[i][j][k-1]=='\\')):
                                variable=""
                            elif tableau_tabPrincipale[i][j][k] == str(parametres._parametreImage):
                                variable="image"
                                images.append(tableau_tabPrincipale[i][j][k])
                                nImages+=1
                            elif tableau_tabPrincipale[i][j][k] == str(parametres._parametreVideo):
                                variable="video"
                                print("video:",video)
                                video.append(tableau_tabPrincipale[i][j][k])
                                nVideo+=1
                            else:
                                variable="titre"
                            ...
                            (tableau_tabPrincipale[i][j][k]=='n' and tableau_tabPrincipale[i][j][k-1]=='\\')):
                                ...
                                images[len(commentaire)-1]+=tableau_tabPrincipale[i][j][k]
                            elif variable=="video":
                    ...
                    if len(Titre)>0:
                        ...
                        for z in range(len(Titre)):
                            valeur_petit_fils+=('" valeur_'+str(z)+'="'+Titre[z])
                        ...
                        print(len(video))
                        if len(video)>0:
                            print("affichage video")
                            for k in range(len(video)):
                                valeur_petit_fils+=(' video_'+str(k)+'="'+video[k])
                                print("video:",video)
                            valeur_petit_fils+='"'
                        if len(images) > 0:
                            for k in range(len(images)):
                                valeur_petit_fils+=(' image_'+str(k)+'="'+images[k])
                            valeur_petit_fils+='"'
